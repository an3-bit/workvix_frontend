...
const ManageJobs: React.FC = () => {
  ...
  return (
    <div className="p-6 bg-background min-h-screen pb-8">
      {/* Sidebar */}
      <aside className={`transition-all duration-200 bg-white border-r ${sidebarOpen ? 'w-full md:w-64' : 'w-16'} p-4 flex flex-col items-center md:items-stretch min-h-screen`}>
        <button
        ...
        <nav className="space-y-2 w-full">
          {jobStatusFilters.map(filter => (
            <button
              key={filter.key}
              className={`flex items-center w-full px-3 py-2 rounded-lg transition font-medium text-left ${activeTab === filter.key ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100 text-gray-700'} ${!sidebarOpen ? 'justify-center' : ''}`}
      ...
      {/* Main Content */}
      <main className="flex-1 p-4 md:p-8 flex flex-col min-h-screen">
        <div className="max-w-6xl mx-auto w-full flex flex-col h-full">
          <h1 className="text-3xl font-bold mb-6 flex items-center"><Briefcase className="w-7 h-7 mr-2 text-blue-600" /> Manage Jobs</h1>
          ...
          <div className="flex-1 overflow-y-auto max-h-[calc(100vh-120px)]"> {/* Adjust 120px if your header is taller/shorter */}
            ...
            <div className="hidden sm:block">
              <Card className="border border-gray-200 shadow-2xl rounded-2xl bg-white/90 backdrop-blur-md animate-fade-in">
                <CardHeader className="flex flex-row flex-wrap items-start justify-between gap-2 p-4 sm:p-6">
                  <CardTitle className="font-bold text-lg sm:text-2xl text-gray-800 w-full sm:w-auto break-words whitespace-normal m-0 p-0">
                      ...
                      <Table className="min-w-full">
                        ...
                        <TableBody>
                          {filteredJobs.map((job, idx) => (
                            <TableRow key={job.id} className={`transition-all duration-300 ${idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-blue-50/60 hover:shadow-md animate-fade-in-row`}>
