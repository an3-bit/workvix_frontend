...
const ClientDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch jobs
      const jobsData = await api.jobs.getAll({ client_id: user.id });

      // Fetch bids for client's jobs
      const bidsData = await api.bids.getAll({ 
        job_ids: jobsData?.map(job => job.id) || [] 
      });
      ...
      // For now, we'll use empty array until backend implements this
      setSpendingData([]);
      ...
      const fetchRecentPayments = async () => {
        if (!user?.id) return;
        const payments = await api.payments.getHistory();
        if (payments && payments.length > 0) setRecentPayments(payments.slice(0, 5));
      };
      fetchRecentPayments();
    } catch (err: any) {
